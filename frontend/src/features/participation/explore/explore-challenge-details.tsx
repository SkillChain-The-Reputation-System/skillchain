...
export default function ExploreChallengeDetails({
  ...
  async function handleJoinChallenge() {
    ...
    try {
      setJoining(true);
      const txHash = await userJoinChallenge(
        Number(challenge_id),
        address,
        challengeCost
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const fetchHasJoinedState = address
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction
                    ...
                    disabled={joining || challengeCost <= 0 || costLoading}
                  >
                    {joining ? (
                      <div className="flex items-center gap-2">
                        <LoaderCircle className="h-4 w-4 animate-spin" />
                ...
                {hasJoined ? (
                  <>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleGoToWorkspace(challenge_id.toString())
                      }
                    >
                      <CheckCircle2 className="h-4 w-4" />
                      Go to Workspace
